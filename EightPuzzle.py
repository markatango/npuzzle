...
##    def nextPuzzle(self):
##        """ find the best puzzle based on random selection of puzzles with the minimum heuristic,
##not already visited"""
##        possible_puzzles = self.children()
##        PP_hashes = map(hash, possible_puzzles)
##      #  for p in PP_hashes[0:2]:
##      #      EightPuzzle.visited_puzzles.append(p)
##        
##        new_puzzles = [not p in EightPuzzle.visited_puzzles for p in PP_hashes]
##        unvisited_puzzles = []
##        for b in range(len(new_puzzles)):
##            if new_puzzles[b]:
##                unvisited_puzzles.append(possible_puzzles[b])
##        print 'number of possible puzzels not visited: %s' % len(unvisited_puzzles)
##        scores = []
##        for p in unvisited_puzzles:
##            scores.append(EightPuzzle(p).heuristic())
##        min_score = min(scores)
##        min_puzzle_ind = [s == min_score for s in scores]
##        num_cands = len(min_puzzle_ind)
##        final_puzzles = []
##        for s in range(num_cands):
##            if min_puzzle_ind[s]:
##                final_puzzles.append(unvisited_puzzles[s])
##        final_puzzle = r.choice(final_puzzles)
##        EightPuzzle.visited_puzzles.append(hash(final_puzzle))
##        print 'final puzzle:'
##        return final_puzzle
##            
